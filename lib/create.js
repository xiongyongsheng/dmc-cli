...
class Creator {
  ...
  async downloadTemplate(choiceTemplateName) {
    ...
    const rootPath = path.join(cwd, this.projectName);
    loading.start();
    await this.downloadGitRepo(templateUrl, rootPath);
    if (fs.existsSync(`${rootPath}/package.json`)) {
      const packageJSON = fs.readJSONSync(`${rootPath}/package.json`);
      packageJSON.name = this.projectName;
      fs.writeJsonSync(`${rootPath}/package.json`, packageJSON, {
        spaces: 2,
      });
    }
    if (fs.existsSync(`${rootPath}/package-lock.json`)) {
      fs.removeSync(`${rootPath}/package-lock.json`);
    }
